...
export default function Polls({poll}:{poll:Message}) {
 ...
 if(poll.options) return (
  <div className={`flex flex-col shadow-md p-2 pl-4 rounded  border border-stone-700 w-full  justify-center ${poll.displayname !== Info.displayname? "bg-slate-800":"bg-slate-800"}  `}><div className="flex justify-between w-full h-full items-center "><div className="text-sm text-slate-400 font-bold">{poll.displayname}</div><div className="text-xs"><MsgAction message={poll.message} chatId={poll.id} displayname={poll.displayname}/></div></div>
    <div className="font-bold flex flex-col gap-1 text-white mt-1   ">{poll.message}</div>
    {poll.multipleVotes ? <div className="flex items-center text-xs  mt-1"><TiTick className="rounded-full text-white bg-slate-600"/><TiTick className="rounded-full text-white bg-slate-600 mr-1" /> Select one or more</div>: <div className="text-xs flex mt-1 items-center "><TiTick className="rounded-full text-white bg-slate-600 mr-1" />Select one</div>}
    ...
    <div className="flex w-full justify-center pt-2 border-t border-t-gray-400/30 mt-1 font-bold hover:cursor-pointer hover:text-slate-400" onClick={()=>setChatType(ChatType.VOTES)}>View Votes</div>
    </div>
  )
}