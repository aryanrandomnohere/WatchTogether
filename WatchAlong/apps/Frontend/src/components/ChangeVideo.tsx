...
export default function ChangeVideo() {
  ...
  function handleChangeVideo(e: FormEvent) {
    ...
    socket.emit('change-video', {
      playing: {
        id: inputPlaying.id.toString(),
        title: inputPlaying.title,
        type: inputPlaying.type,
        animeId: inputPlaying.animeId,
      },
      roomId,
    });
  ...
  return (
    <form onSubmit={handleChangeVideo} className="flex">
      <div className="flex flex-col gap-2 p-6">
        <div>
          <h1 className="text-sm text-slate-400">Media Type</h1>
          <select
            className="text-xs rounded-lg py-1 px-3 font-medium w-36 bg-opacity-20 text-white bg-slate-950  focus:outline-none"
            onChange={e => handleInputChange({ type: e.target.value })}
            value={inputPlaying.type}
            required={true}
          >
            <option>AniMov</option>
            <option>Url</option>
            <option>Movie</option>
            <option>Series</option>
            <option>Anime</option>
            <option>AnimeUrl</option>
          </select>
        </div>
        {inputPlaying.type === 'AnimeUrl' && (
          <div>
            ...
            <input
              ...
              value={inputPlaying.animeId}
              onChange={e => handleInputChange({ animeId: e.target.value })}
              required={true}
              placeholder="Src Url"
            />
          </div>
      ...
      <button className="p-1 text-sm hover:bg-slate-600 border-l px-3 border-slate-400 hover:text-white font-bold ">
        Change
      </button>
    </form>
  );
