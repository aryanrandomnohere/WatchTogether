...
export default function Option({ option, totalVotes }: { option: Option; totalVotes: number }) {
  ...
  const isSelected =
    Array.isArray(option.votes) && option.votes.some(vote => vote.user.id === Info.id);
  ...
  return (
    <div className="flex w-full text-white items-center hover:cursor-pointer mt-2">
      <div onClick={handleAddVote}>
        {isSelected ? (
          <FaCheckCircle className="text-xl text-slate-400" />
        ) : (
      ...
      <div className="flex flex-col justify-center w-full">
        ...
        <div className="relative  w-68 h-2 bg-gray-800  rounded-full ml-2">
          <div
            className="absolute h-full bg-yellow-100 transition-all duration-700 ease-in-out rounded-full"
