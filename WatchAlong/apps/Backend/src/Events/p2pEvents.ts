...
export default function p2pEvents(io: Server, socket: Socket) {
  ...
  socket.on("initiate-offer", (roomId: string, userInfo, to: string | null, sdp: RTCSessionDescriptionInit) => {
    console.log(`User ${userInfo.id} initiating offer in room ${roomId}${to ? ` to user ${to}` : ''}`);
    ...
    const joined = roomManager.getInstance().joinCall(roomId, userInfo.id, sdp);
  ...
  socket.on("answer-created", (roomId: string, userId: string, toUserId: string, answer: RTCSessionDescriptionInit) => {
    console.log(`User ${userId} sending answer to ${toUserId} in room ${roomId}`);
