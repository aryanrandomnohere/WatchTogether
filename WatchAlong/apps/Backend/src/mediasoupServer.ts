...
io.on('connection', (socket) => {
  ...
  socket.on('connectTransport', async ({ roomId, peerId, transportId, dtlsParameters }) => {
    try {
      const room = rooms.get(roomId);
      if (!room) throw new Error('Room not found');
  ...
  socket.on('produce', async ({ roomId, peerId, transportId, kind, rtpParameters }) => {
    try {
      const room = rooms.get(roomId);
      if (!room) throw new Error('Room not found');
  ...
  socket.on('consume', async ({ roomId, peerId, transportId, producerId, rtpCapabilities }) => {
    try {
      const room = rooms.get(roomId);
      if (!room) throw new Error('Room not found');
