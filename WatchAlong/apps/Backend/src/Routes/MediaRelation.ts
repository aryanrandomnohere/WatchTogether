...
MediaRouter.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(500)
        .json({ msg: "An error occurred while retrieving media." });
    }
  },
);
...
MediaRouter.put(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      const { movieId, listType }: { movieId: number; listType: string } =
        req.body;
      ...
      res
        .status(500)
        .json({ msg: "An error occurred while saving the movie." });
    }
  },
);
    ...
    try {
      ...
      if (!userId || !movie || !listType) {
        res
          .status(400)
          .json({
            msg: "Missing required fields: userId, movie, or listType.",
          });
        return;
      }
      ...
      if (!VALID_LIST_TYPES.includes(listType)) {
        res
          .status(400)
          .json({
            msg: `Invalid listType. Valid options are: ${VALID_LIST_TYPES.join(", ")}`,
          });
        return;
      }
      ...
      res
        .status(201)
        .json({
          message: "Movie successfully added to the list.",
          movie: movieRecord,
        });
    } catch (error) {
      console.error(error);
  ...
  async (req: UpdateEpisodeRequest, res: Response) => {
    try {
      ...
      const {
        ...
        season,
        movie_Id,
      }: { episode: number; season: number; movie_Id: number } = req.body;
      ...
      if (!episode || !season || !movie_Id || episode <= 0 || season <= 0) {
        res
          ...
          .json({
            success: false,
            message:
              "Invalid input: episode, season, and movieId are required and must be valid numbers.",
          });
        return;
      }
      ...
      if (response) {
        ...
        res
          ...
          .json({
            success: true,
            message: "Episode and season updated successfully.",
          });
        return;
      }
      ...
      res
        ...
        .json({
          success: true,
          message: "Movie relation made and episode and season updated",
        });
      return;
    } catch (error) {
